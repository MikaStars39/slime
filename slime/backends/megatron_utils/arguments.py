...
def validate_args(args):
    ...
    # MuonProjected optimizer checks
    ...
    # Shampoo optimizer checks
    if getattr(args, 'optimizer', 'adam') == 'shampoo':
        assert not args.use_distributed_optimizer, "Shampoo does not support distributed optimizer."
        assert not getattr(args, 'fp16', False), "Shampoo does not support fp16, use bf16 instead."
...
def _set_default_megatron_args(args):
    ...
    if 'muon' in opt or opt in ('roo', 'sgd', 'muon_projected', 'gasd', 'soap', 'shampoo', 'rmsprop'):
